...
const testPolicy = definePolicy<TestUser, TestResource>({
  update: (user, resource) => user.id === resource?.ownerId,
  delete: (user) => user.role === 'admin',
});

describe('Gate', () => {
  const owner: TestUser = { id: 'user-1', role: 'member' };
  const admin: TestUser = { id: 'user-2', role: 'admin' };
  const resource: TestResource = { ownerId: 'user-1' };
  ...
  it('AuthorizationError message is always "Unauthorized"', async () => {
    ...
    try {
      ...
      expect((err as AuthorizationError).message).toBe('Unauthorized');
    }
  });
...
describe('Gate.register / Gate.resolve', () => {
  ...
  it('overwrites a previously registered policy with the same name', () => {
    const policyA = definePolicy<TestUser, TestResource>({ update: () => true });
    const policyB = definePolicy<TestUser, TestResource>({ update: () => false });
