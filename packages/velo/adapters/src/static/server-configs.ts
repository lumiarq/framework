export interface ApacheOptions {
  /**
   * Custom rewrite rules to include before the SPA fallback.
   * Each entry is inserted verbatim, one per line.
   */
  extraRules?: string[];
}

export interface NginxOptions {
  /** Port nginx should listen on (default: 80) */
  port?: number;
  /** Document root path (default: /usr/share/nginx/html) */
  root?: string;
}
 ...
 */
export function generateHtaccess(options: ApacheOptions = {}): string {
