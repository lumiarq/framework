...
function colorizeLevel(level: LogEntry['level']): string {
  switch (level) {
    case 'debug':
...
function stringifyContext(entry: LogEntry, pretty: boolean): string {
...
export function createConsoleChannel(options: ChannelOptions = {}): Channel {
  return {
