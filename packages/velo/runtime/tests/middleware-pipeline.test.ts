...
describe('composeMiddleware', () => {
  ...
  it('executes middleware in order before the handler', async () => {
    ...
    const mw1 = async (_req: Request, next: () => Promise<Response>) => {
      order.push('mw1-before');
      const res = await next();
      order.push('mw1-after');
      return res;
    };
    const mw2 = async (_req: Request, next: () => Promise<Response>) => {
  ...
  it('middleware can short-circuit without calling next()', async () => {
    ...
    const blocker = async (_req: Request, _next: () => Promise<Response>) =>
  ...
  it('all middleware headers accumulate on the final response', async () => {
    const addA = async (_req: Request, next: () => Promise<Response>) =>
      withHeaders(await next(), { 'X-A': 'a' });
    const addB = async (_req: Request, next: () => Promise<Response>) =>
      withHeaders(await next(), { 'X-B': 'b' });
