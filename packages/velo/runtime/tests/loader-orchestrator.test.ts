...
describe('orchestrateLoaders — parallel execution', () => {
  it('runs independent loaders in parallel (total time ≈ slowest, not sum)', async () => {
    const delay = (ms: number, v: unknown) => new Promise((r) => setTimeout(() => r(v), ms));
...
describe('orchestrateLoaders — dependency ordering (dependsOn)', () => {
  it('runs dependent loader after its dependency resolves', async () => {
    ...
    const permLoader = makeLoader(
      'perms',
      async ({ deps }) => {
        const user = deps.get('user') as { role: string };
        order.push('perms');
  ...
  it('deps Map is populated with resolved dependency data', async () => {
    ...
    const childLoader = makeLoader(
      'child',
      async ({ deps }) => {
        const parent = deps.get('parent') as { value: number };
...
describe('orchestrateLoaders — acyclic DAG validation', () => {
  it('throws LoaderDependencyError immediately on a direct self-reference', async () => {
    const selfRef: BoundLoader = { id: 'self', fetch: async () => null, dependsOn: [] };
    (selfRef as { dependsOn: BoundLoader[] }).dependsOn = [selfRef];
  ...
  it('throws LoaderDependencyError on a two-node cycle (A → B → A)', async () => {
    const loaderA: BoundLoader = { id: 'A', fetch: async () => null, dependsOn: [] };
    const loaderB: BoundLoader = { id: 'B', fetch: async () => null, dependsOn: [loaderA] };
    (loaderA as { dependsOn: BoundLoader[] }).dependsOn = [loaderB];
  ...
  it('error message contains the cycle path', async () => {
    const loaderA: BoundLoader = { id: 'LoaderA', fetch: async () => null, dependsOn: [] };
    const loaderB: BoundLoader = {
      id: 'LoaderB',
      fetch: async () => null,
      dependsOn: [loaderA],
    };
