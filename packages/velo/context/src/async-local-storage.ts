 ...
 */
let applicationContext: ExecutionContext | null = null;
 ...
 */
export function setApplicationContext(ctx: ExecutionContext): void {
 ...
 */
function buildAmbientContext(): ExecutionContext {
  const id = randomUUID();
  const prefix = `[${id}]`;
