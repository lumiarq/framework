...
import { StaticBuildError } from '@illumiarq/core';
...
const isrRoute = (path: string, revalidate: number): RouteDefinition => ({
  path,
  handler,
  render: 'static',
  revalidate,
});
const dynamicRoute = (path: string): RouteDefinition => ({ path, handler, render: 'dynamic' });
...
describe('prerenderRoutes — error handling', () => {
  ...
  it('StaticBuildError message contains the failing route path', async () => {
    ...
    try {
      ...
      expect((err as StaticBuildError).message).toContain('/crash');
    }
  });
});
