...
import { defineRoute, defineSitemapLoader } from '@illumiarq/http';
import type { SitemapEntry } from '@illumiarq/http';
...
const ssgRoute = (path: string): RouteDefinition =>
  defineRoute({ path, handler: h, render: 'static', revalidate: false });

const dynRoute = (path: string): RouteDefinition =>
  defineRoute({ path, handler: h, render: 'dynamic' });
...
describe('generateSitemap — merges static SSG routes with SitemapLoader entries', () => {
  ...
  it('SitemapLoader entry with its own lastmod overrides the global default', () => {
    const sitemapEntries: SitemapEntry[] = [{ url: '/blog/new-post', lastmod: '2025-06-15' }];
  ...
  it('emits <priority> only when SitemapEntry provides it', () => {
    const withPriority: SitemapEntry[] = [{ url: '/important', priority: 0.9 }];
    const withoutPriority: SitemapEntry[] = [{ url: '/normal' }];
  ...
  it('SitemapLoader entries appear after SSG route entries (declaration order preserved)', () => {
    ...
    const sitemapEntries: SitemapEntry[] = [{ url: '/blog/post-1' }];
