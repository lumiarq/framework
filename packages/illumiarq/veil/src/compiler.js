...
function minifyHtml(html) {
    ...
    let result = html.replace(/<(style|script)([\s\S]*?)>([\s\S]*?)<\/\1>|<%[\s\S]*?%>/gi, (match, tag, attrs, content) => {
        let processed = match;
        if (tag) {
    ...
    result = result.replace(new RegExp(`${placeholder}(\\d+)\x00`, 'g'), (_, i) => blocks[Number(i)] ?? '');
    return result;
}
/**
 ...
 */
function resolveAsset(file, assetDirs, sharedAssetDirs) {
...
function resolveExtends(template, templatesDir, sharedTemplatesDirs) {
    ...
    let layoutPath = tryExtensions(join(templatesDir, layoutName)) ??
    ...
    layout = layout.replace(/@yield\('([^']+)'\)/g, (_, name) => {
...
function resolveIncludes(template, templatesDir, sharedTemplatesDirs) {
    return template.replace(/@include\('([^']+)'\)/g, (match, includePath) => {
        let resolved = tryExtensions(join(templatesDir, includePath)) ??
 ...
 */
export function compileTemplate(template, ctx, lang = {}) {
    ...
    const destructure = varsEntries.length > 0 ? `{ ${varsEntries.map((v) => v.name).join(', ')} }` : '{}';
    ...
    template = template.replace(/@styles\('([^']+)'\)/g, (_, file) => {
        const p = resolveAsset(file, assetDirs, sharedAssetDirs);
        if (!p)
            return `<!-- view:cache: styles '${file}' not found -->`;
        return `<style>\n${readFileSync(p, 'utf8')}\n</style>`;
    });
    template = template.replace(/@scripts\('([^']+)'\)/g, (_, file) => {
        const p = resolveAsset(file, assetDirs, sharedAssetDirs);
        if (!p)
            return `<!-- view:cache: scripts '${file}' not found -->`;
        return `<script>\n${readFileSync(p, 'utf8')}\n</script>`;
    });
    ...
    if (hasTypes) {
        ...
        lines.push(`}`, ``);
        lines.push(`export function render(vars: RenderVars, locale: Record<string, string> = {}): string {`);
    }
    else {
        // Untyped path: plain Record<string, any> (backward-compatible)
        lines.push(`// eslint-disable-next-line @typescript-eslint/no-explicit-any`);
        lines.push(`export function render(vars: Record<string, any>, locale: Record<string, string> = {}): string {`);
    }
    // Escaper (inside function — defined once per call, not per module to keep scope clean)
    lines.push(`  // eslint-disable-next-line @typescript-eslint/no-explicit-any`, `  const __e = (v: unknown): string =>`, `    String(v ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')`, `  const __t = (key: string): string => locale[key] ?? key`);
...
export async function viewCache(cwd = process.cwd(), options = {}) {
    const modulesDir = resolve(cwd, APP_PATHS.modulesDir);
    if (!existsSync(modulesDir))
        return { compiled: 0, paths: [] };
    ...
    const lang = existsSync(langPath)
 ...
 */
export async function viewClear(cwd = process.cwd()) {
    const dir = resolve(cwd, APP_PATHS.viewsCache);
    if (!existsSync(dir))
        return { cleared: false, dir };
