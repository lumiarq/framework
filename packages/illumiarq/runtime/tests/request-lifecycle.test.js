...
const echoHandler = async (req) => new Response('ok', { status: 200 });
...
describe('handleRequest — rate limiting', () => {
  ...
  it('returns 429 when the limit is exceeded', async () => {
    ...
    let calls = 0;
    const countingHandler = async (_req) => {
      calls++;
...
describe('handleRequest — AsyncLocalStorage context', () => {
  it('handler can read the request context via getContext()', async () => {
    let requestId;
    const contextCapturingHandler = async (_req) => {
...
//# sourceMappingURL=request-lifecycle.test.js.map
