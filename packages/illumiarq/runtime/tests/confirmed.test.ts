...
function makeStore(): InMemorySessionStore {
...
describe('writeConfirmedAt', () => {
  ...
  it('writes __confirmed_at timestamp to the session', async () => {
    ...
    expect(confirmedAt as number).toBeGreaterThanOrEqual(before);
    expect(confirmedAt as number).toBeLessThanOrEqual(after);
  });
