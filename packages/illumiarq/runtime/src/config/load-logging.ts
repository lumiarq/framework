...
export type RuntimeLoggingChannel = {
  driver?: 'console' | 'file' | 'http' | string;
...
export type RuntimeLoggingConfig = {
  ...
  default?: string;
  prettify?: boolean;
  channels?: Record<string, RuntimeLoggingChannel>;
};

const DEFAULT_LOGGING_CONFIG: RuntimeLoggingConfig = {
  level: 'info',
  driver: 'console',
  default: 'console',
...
function sanitizeLoggingConfig(raw: unknown): RuntimeLoggingConfig {
  if (!raw || typeof raw !== 'object') {
    return { ...DEFAULT_LOGGING_CONFIG };
  }

  const candidate = raw as RuntimeLoggingConfig;
...
async function loadConfigModule(filePath: string): Promise<RuntimeLoggingConfig> {
  const url = pathToFileURL(filePath);
  const module = await import(url.href);
  return sanitizeLoggingConfig(module.default ?? module.logging ?? module);
}

function loadJsonConfig(filePath: string): RuntimeLoggingConfig {
...
export async function loadLoggingConfig(projectRoot?: string): Promise<RuntimeLoggingConfig> {
...
export function getDefaultLoggingConfig(): RuntimeLoggingConfig {
  return { ...DEFAULT_LOGGING_CONFIG };
}