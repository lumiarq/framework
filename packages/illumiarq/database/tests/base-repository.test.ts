...
class TestRepository extends BaseRepository<{ id: string; name: string }, { name: string }> {
  ...
  testClampLimit(requested?: number, max?: number) {
...
describe('BaseRepository — connection factory', () => {
  ...
  it('throws a descriptive error when config is not set', () => {
    ...
    const repoWithBadConn = new (class extends TestRepository {
      protected override readonly connection = 'nonexistent';
    })();
