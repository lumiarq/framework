import { getTestTransaction } from '@illumiarq/core';
...
export interface IRepository<TModel, TCreateInput, TUpdateInput = Partial<TCreateInput>> {
  ...
  findAll(opts?: { limit?: number; offset?: number }): Promise<TModel[]>;
...
export interface PaginateOptions {
  ...
  perPage: number; // clamped to [1, maxPerPage]
}
...
export interface PaginatedResult<T> {
  ...
  perPage: number;
  lastPage: number;
  from: number; // 1-indexed (0 when empty)
  to: number; // 1-indexed (0 when empty)
}

export interface CursorPaginateOptions {
  cursor?: string; // base64-encoded position cursor; absent = first page
  limit: number; // clamped to [1, maxPerPage]
}
...
> implements IRepository<TModel, TCreateInput, TUpdateInput> {
   ...
   */
  protected readonly connection: string = 'default';
  ...
  protected get db(): LibSQLDatabase {
    ...
    return (testDb as LibSQLDatabase | undefined) ?? getConnection(this.connection);
  }
   ...
   */
  protected clampLimit(requested?: number, max = 100): number {
  ...
  abstract findAll(opts?: { limit?: number; offset?: number }): Promise<TModel[]>;
