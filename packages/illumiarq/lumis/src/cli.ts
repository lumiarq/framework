...
async function main(): Promise<number> {
  ...
  if (cmd === 'down') {
    ...
    return enableMaintenanceMode({ ...(message ? { message } : {}), ...(secret ? { secret } : {}), ...(allowedIps.length > 0 ? { allowedIps } : {}) })
  }
  ...
  if (cmd === 'auth:install') {
    const uiFramework = parseStringFlag(argv, '--ui') as 'react' | 'vue' | 'svelte' | 'solid' | undefined
    return installAuth({ iam: argv.includes('--iam'), ...(uiFramework ? { ui: uiFramework } : {}) })
  }
