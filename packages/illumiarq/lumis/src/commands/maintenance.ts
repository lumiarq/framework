...
export function enableMaintenanceMode(options: MaintenanceOptions = {}, cwd = process.cwd()): number {
  const targetPath = join(cwd, '.maintenance')
  const payload = {
    ...(options.message ? { message: options.message } : {}),
    ...(options.secret ? { secret: options.secret } : {}),
    ...(options.allowedIps && options.allowedIps.length > 0 ? { allowedIps: options.allowedIps } : {}),
