...
function runBridge(args: string[], cwd: string): { status: number; stdout: string; stderr: string } {
...
function writeGeneratedFiles(cwd: string, files: Array<{ path: string; content: string }>): { created: number; skipped: number } {
  let created = 0
  let skipped = 0
...
export function listRoutes(cwd = process.cwd()): number {
  ...
  const parsed = JSON.parse(result.stdout) as {
    routes: Array<{ method: string; path: string; name: string; render: string; middleware: string[] }>
  }
...
export function runDatabaseCommand(commandName: string, args: string[], cwd = process.cwd()): number {
