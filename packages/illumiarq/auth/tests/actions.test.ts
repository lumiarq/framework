...
function makeSessionRepo(initial: Session[] = []): ISessionRepository & { store: Session[] } {
  const store: Session[] = [...initial];
  return {
    store,
...
describe('BaseRegisterAction', () => {
  ...
  it('does NOT create a session (login action handles that)', async () => {
    ...
    expect((result as unknown as { session?: unknown }).session).toBeUndefined();
    expect((result as unknown as { jwt?: unknown }).jwt).toBeUndefined();
  });
});
...
describe('BaseResetPasswordAction', () => {
  ...
  it('new password verifies correctly after reset', async () => {
    ...
    const updated = await identityRepo.findById(identity.id);
    expect(
      await VerifyPasswordTask({ plain: 'my-new-pass-123', hash: updated!.passwordHash }),
    ).toBe(true);
  });
});
